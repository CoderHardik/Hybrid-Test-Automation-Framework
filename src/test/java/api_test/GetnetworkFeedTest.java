...
public class GetnetworkFeedTest {
		
	// 1. Declare the variables here (but do NOT assign values yet)
    private String testEmail;
    private String CI_ADAS_Filter_value;
    private String user_original_id_value;
    private String user_guid_value;
    private String URL;
    private String token;

    @BeforeClass
    public void setUp() {
        // 2. Assign all values here. This is safe and professional.
        this.testEmail = "test.user@example.com"; 
        this.URL = AuthenticationToken.getBaseUri();
        this.token = AuthenticationToken.getToken();
        // Retrieve sanitized data from UserTable
        this.CI_ADAS_Filter_value = UserTable.get_ci_filter().get(testEmail);
        this.user_original_id_value = UserTable.user_original_id().get(testEmail);
        this.user_guid_value = UserTable.user_guid().get(testEmail);
        
        System.out.println("Setup complete. Token retrieved for: " + testEmail);
    }
	...
	@Test
	public void getMinedEmail() throws InterruptedException{
	
		given()
		.header("Authorization", "Bearer "+token).
		header("activity_type","email").
		header("tenant_id",16002).
		header("thread_id","169c99708358380c").
		header("source_type","google").
		header("message_id","169ddaa612c67e8d").
		header("opp_id","000OppX5c800f18a0706208bb201f97").
		
		when()
		.get(URL+"/analytics/rest/v2/analysis/activitymessagedetails").
		
		then()
		.statusCode(200)
		.extract().response().prettyPrint();	
		 	
	}//This will throw 500 error as expected. This should grab mined email for specific opportunity. Need to make it generic.
